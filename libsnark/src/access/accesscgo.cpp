...
r1cs_gg_ppzksnark_proof<ppzksnark_ppT> generate_access_proof(r1cs_gg_ppzksnark_proving_key<ppzksnark_ppT> proving_key,
                                                        uint256 id,
                                                        uint256 cmtU,
                                                        uint256 token,
                                                        uint256 pk,
                                                        ...
                                                        )
{
    ...
    { // 三元组R1CS是否满足  < A , X > * < B , X > = < C , X >
        cout << "can not generate access proof" << endl;
                   ...
                   )
{
    ...
    uint256 token = uint256S(token_string);
    ...
    keypair.pk = deserializeProvingKeyFromFile("/usr/local/prfKey/accesspk.raw");

    gettimeofday(&t2,NULL);
    timeuse = t2.tv_sec - t1.tv_sec + (t2.tv_usec - t1.tv_usec)/1000000.0;

    // 生成proof
    libsnark::r1cs_gg_ppzksnark_proof<libff::alt_bn128_pp> proof = generate_access_proof<alt_bn128_pp>(keypair.pk, id, cmtU, token, pk, ek, r, rt);
    ...
    char *token_string)
{
    ...
    r1cs_gg_ppzksnark_keypair<alt_bn128_pp> keypair;
    keypair.vk = deserializevkFromFile("/usr/local/prfKey/accessvk.raw");

    gettimeofday(&t2,NULL);
    timeuse = t2.tv_sec - t1.tv_sec + (t2.tv_usec - t1.tv_usec)/1000000.0;
    // printf("\n\n reading access vk Use Time:%fs\n\n",timeuse);
    ...
    bool result = verify_access_proof(keypair.vk, proof, id, cmtU, token);

    if (!result)
    {
        cout << "Verifying access proof failed" << endl;
    }
    else
    {
